...
def build_thread_context(client, channel_id, thread_ts, fallback_text=""):
    messages = []
    try:
        replies = client.conversations_replies(channel=channel_id, ts=thread_ts, inclusive=True, limit=50)
...
def process_task_request(client, say, channel_id, thread_ts, raw_text, slack_user_id):
    try:
        ...
        say(f"✅ Tarefa criada no *seu* Trello!\n📌 *{card['name']}*{due}{prio}{img_note}\n🔗 <{card['url']}|Abrir card>", thread_ts=thread_ts)
    except Exception as e:
...
def handle_mention(body, say, client):
    ...
    threading.Thread(
        ...
        args=(client, say, event.get("channel"), event.get("thread_ts") or event.get("ts"), event.get("text", ""), event.get("user")),
        daemon=True,
    ).start()
...
def trello_callback():
    ...
    if not slack_user_id:
        return "slack_user_id é obrigatório", 400
    return render_template_string(TRELLO_CALLBACK_PAGE, app_name=APP_NAME, css=PAGE_CSS, slack_user_id=slack_user_id)
...
def google_callback():
    ...
    try:
        ...
        expiry_dt = datetime.fromtimestamp(datetime.now(timezone.utc).timestamp() + expires_in, tz=timezone.utc)
        email = None
        try:
...
def trello_select_list():
    ...
    try:
        ...
        for b in boards_raw:
            if b.get("closed"):
                continue
            lists = [lst for lst in trello_lists(user["trello_token"], b["id"]) if not lst.get("closed")]
            for lst in lists:
                ...
                if preferred is None and ("resumeai" in name_b or name_l in ("a fazer", "to do", "todo", "inbox")):
